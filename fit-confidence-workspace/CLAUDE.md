...
Workspace này được cấu hình theo quyết định của user là **MONOREPO**: toàn bộ code subproject (`frontend/`, `backend/`, `storefront/`) được **commit chung** vào git repo `harness-demo` (boilerplate), push lên `https://github.com/hieuxipat/harness-demo`. Khác với mặc định (mỗi subproject là git repo riêng, overlay local-only qua `.git/info/exclude`):

- Subproject **không có `.git` riêng** — đã xoá khi clone để gộp monorepo.
- Harness/codegraph artifacts không dùng `.git/info/exclude`; thay vào đó ignore qua `fit-confidence-workspace/.gitignore`: `.codegraph/`, `.claude/state/`, `state.db`, `evidence/`.
