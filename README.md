...
**AI-powered development workflow** cho division Megamind. Boilerplate cung cấp 17 Claude Code skills giúp dev tự động hoá quy trình làm task: từ phân tích task, chia nhỏ user stories, implement theo TDD, review code, test, đến thông báo team qua Lark.
...
```
megamind-{app-name}/
├── .claude/skills/           # 17 skills (não bộ của workflow)
├── backend/                  # NestJS app
...
|---|---|---|
| `/init-workspace` | Khởi tạo workspace mới từ boilerplate | Chạy 1 lần sau khi clone (tự xoá sau) |
...
| Skill | Mục đích | Khi nào dùng |
|---|---|---|
| `/explore-task` | Lấy task từ board (Notion/Linear/Lark/Jira), filter + lưu file | Cần xem task nào cần làm |
...
- **Resume-friendly** — Nếu workflow bị gián đoạn, gọi lại `/task-flow` sẽ đọc registry và resume từ bước cuối
- **Multi-dev safe** — Mỗi dev sở hữu stories riêng, registry contract đảm bảo không conflict (xem `REGISTRY-CONTRACT.md`)
...
- `docs/REGISTRY-CONTRACT.md` — Quy tắc concurrent access cho Feature Registry (multi-dev)
- `docs/features/_example-cookie-banner/` — Feature mẫu hoàn chỉnh (manifest, user stories, test cases, coverage matrix, ADR)
