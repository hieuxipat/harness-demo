# Task Explorer Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Claude Code skill that explores and filters tasks from external task management tools (Lark, Jira) via playwright-cli browser automation, saving results as individual markdown files.

**Architecture:** Single skill file (`skill.md`) containing all instructions for Claude to follow. No code — purely declarative skill instructions leveraging the existing `playwright-cli` skill for browser automation.

**Tech Stack:** Claude Code skill (markdown), playwright-cli

---

## File Structure

- Create: `.claude/skills/task-explorer/skill.md` — the skill definition
- Remove: `.claude/skills/task-explorer/.gitkeep` — no longer needed

---

### Task 1: Write the task-explorer skill file

**Files:**
- Create: `.claude/skills/task-explorer/skill.md`

- [ ] **Step 1: Create the skill file with frontmatter**

Write `.claude/skills/task-explorer/skill.md` with the full skill content:
...
```bash
rm .claude/skills/task-explorer/.gitkeep
```

- [ ] **Step 3: Verify the skill file**

```bash
cat .claude/skills/task-explorer/skill.md | head -5
```
...
```bash
git add .claude/skills/task-explorer/skill.md
git rm .claude/skills/task-explorer/.gitkeep
git commit -m "feat(skill): add task-explorer skill for browsing external task boards"
```
